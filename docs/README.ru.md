...
Один результат генерации сразу охватывает **iPhone / iPad, Android, Windows, macOS и Linux**, и каждый из них занимает всего несколько КБ — поэтому скачивается и устанавливается почти мгновенно.
...
Каждый пакет — это лишь тонкая точка входа на ваш сайт: он не упаковывает содержимое сайта, поэтому артефакты измеряются в **килобайтах, а не мегабайтах**. Под капотом используется нативная лёгкая оболочка каждой платформы: WebView APK на Android, профиль Web Clip на iOS и лаунчеры `.app` / `.bat` / `.desktop`, которые открывают системный браузер в режиме приложения на десктопе.

Измерено на реальной сборке (значения репрезентативны и почти не зависят от сайта):

| Платформа | Пакет | Типичный размер | Содержимое |
| --- | --- | --- | --- |
| Android | `android.apk` | **~21 КБ** | Настоящий устанавливаемый WebView APK (подпись v1+v2+v3) |
| iOS / iPadOS | `ios.mobileconfig` | **~4 КБ** | Профиль конфигурации Web Clip |
| macOS | `macos.zip` | **~1,4 КБ** | Бандл `.app` (скрипт-лаунчер + значок) |
| Windows | `windows.zip` | **~1,2 КБ** | Лаунчер `.bat` + помощник для ярлыка + значок |
| Linux | `linux.tar.gz` | **~0,7 КБ** | Запись `.desktop` + скрипт установки + значок |
