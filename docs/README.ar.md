...
نتيجة واحدة مُولّدة تغطي **iPhone / iPad وAndroid وWindows وmacOS وLinux**، وكلٌّ منها بحجم بضعة كيلوبايت فقط — لذا يُنزَّل ويُثبَّت في لحظة تقريبًا.
...
كل حزمة هي مجرد نقطة دخول رفيعة إلى موقعك — لا تُضمِّن أي محتوى للموقع، لذا تُقاس المخرجات بالـ **كيلوبايت لا الميجابايت**. وتعتمد في الأساس على القشرة الخفيفة الأصلية لكل منصة: ‏APK بنمط WebView على أندرويد، وملف Web Clip على iOS، ومُشغّلات `.app` / `.bat` / `.desktop` تفتح متصفح النظام في وضع التطبيق على سطح المكتب.

مقيسة على بناء حقيقي (الأرقام تمثيلية ولا تكاد تتغير حسب الموقع):

<div dir="rtl">

| المنصة | الحزمة | الحجم النموذجي | المحتوى |
| --- | --- | --- | --- |
| Android | `android.apk` | **~21 كيلوبايت** | APK حقيقي قابل للتثبيت بنمط WebView (موقّع v1+v2+v3) |
| iOS / iPadOS | `ios.mobileconfig` | **~4 كيلوبايت** | ملف تكوين Web Clip |
| macOS | `macos.zip` | **~1.4 كيلوبايت** | حزمة `.app` (سكربت تشغيل + أيقونة) |
| Windows | `windows.zip` | **~1.2 كيلوبايت** | مُشغّل `.bat` + مساعد اختصار سطح المكتب + أيقونة |
| Linux | `linux.tar.gz` | **~0.7 كيلوبايت** | مدخل `.desktop` + سكربت تثبيت + أيقونة |
