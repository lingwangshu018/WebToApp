...
1 回の生成結果で **iPhone / iPad、Android、Windows、macOS、Linux** をまとめてカバーします。各成果物はわずか数 KB なので、ダウンロードもインストールもほぼ一瞬です。
...
各パッケージは、あなたのサイトを開くための極薄の入口にすぎず、サイトのコンテンツは同梱しません。そのため成果物のサイズは **MB ではなく KB** で測られます。内部では各プラットフォーム固有の軽量シェルを使います：Android の WebView APK、iOS の Web Clip プロファイル、デスクトップではシステムブラウザをアプリモードで開く `.app` / `.bat` / `.desktop` ランチャーです。

実際のビルドで計測（代表的な値であり、サイトによってほとんど変わりません）：

| プラットフォーム | パッケージ | 典型的なサイズ | 中身 |
| --- | --- | --- | --- |
| Android | `android.apk` | **約 21 KB** | 実際にインストール可能な WebView APK（v1+v2+v3 署名） |
| iOS / iPadOS | `ios.mobileconfig` | **約 4 KB** | Web Clip 構成プロファイル |
| macOS | `macos.zip` | **約 1.4 KB** | `.app` バンドル（起動スクリプト + アイコン） |
| Windows | `windows.zip` | **約 1.2 KB** | `.bat` ランチャー + デスクトップショートカット作成 + アイコン |
| Linux | `linux.tar.gz` | **約 0.7 KB** | `.desktop` エントリ + インストールスクリプト + アイコン |
