...
BUBBLEWRAP_BUILD_TOOLS_VERSION="${BUBBLEWRAP_BUILD_TOOLS_VERSION:-36.1.0}"
PLATFORM_VERSION="${ANDROID_PLATFORM_VERSION:-android-34}"
APKTOOL_VERSION="${APKTOOL_VERSION:-2.11.1}"
BUBBLEWRAP_VERSION="${BUBBLEWRAP_VERSION:-1.25.0}"
...
# Bubblewrap's AndroidSdkTools.validatePath() only accepts SDK roots containing
# either <sdk>/tools or <sdk>/bin. Modern Android command-line tools live under
# <sdk>/cmdline-tools/latest/bin, so expose a compatibility symlink at the SDK
# root instead of making Bubblewrap reject an otherwise valid SDK installation.
if [ ! -e "$SDK_ROOT/bin" ]; then
...
"$SDK_ROOT/cmdline-tools/latest/bin/sdkmanager" --sdk_root="$SDK_ROOT" \
  ...
  "build-tools;${BUBBLEWRAP_BUILD_TOOLS_VERSION}" \
  "platform-tools"
...
echo "sdkmanager=$SDK_ROOT/bin/sdkmanager"
...
echo "bubblewrap-build-tools=$SDK_ROOT/build-tools/${BUBBLEWRAP_BUILD_TOOLS_VERSION}"
